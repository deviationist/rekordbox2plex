...
class PlaylistSync:
    ...
    def sync(self) -> None:
        ...
        if rb_playlists is False or (rb_playlists_count := len(rb_playlists)) == 0:
            ...
            with progress_instance() as progress:
                ...
                progress.update(task, description="[bold green]✔ Done! Rekordbox playlists are synchronized with Plex!")
        self.delete_orphaned_playlists(rb_playlists or [], plex_playlists)
        logger.info(f"[bold green]✔ Result: {self.deleted} deleted, {self.updated} updated, and {self.created} created.")
    ...
    def delete_orphaned_playlists(self, rb_playlists: List[RekordboxPlaylist], plex_playlists: PlexPlaylists):
        orphaned_playlists = []
